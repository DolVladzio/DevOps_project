...
resource "azurerm_linux_virtual_machine" "main" {
	...
	size                = var.vm_instances_size_map[0][each.value.size]
	...
	source_image_reference {
		publisher 		= each.value.os_image[0].publisher
		offer     		= each.value.os_image[0].offer
		sku       		= each.value.os_image[0].sku
		version  		= each.value.os_image[0].version
	}
